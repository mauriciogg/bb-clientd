...
func (d *BazelOutputServiceDirectory) Clean(ctx context.Context, request *bazeloutputservice.CleanRequest) (*bazeloutputservice.CleanResponse, error) {
	...
	d.lock.Unlock()
	if ok {
		// OUT-OF-BAND: wholesale wipe of an output base's tree, bypassing the
		// mount. Fires a NotifyRemoval per child (logged by the removal notifier).
		log.Printf("[OOB DELETE] clean output base %q (remove all children)", request.OutputBaseId)
...
func (d *BazelOutputServiceDirectory) findMissingAndRemove(ctx context.Context, queue map[digest.Digest][]func() error) error {
	...
	for _, digest := range missing.Items() {
		// OUT-OF-BAND: StartBuild evicts outputs whose blobs are gone from the
		// CAS, directly on the tree (bypassing the mount). Each removeFunc fires a
		// NotifyRemoval the removal notifier logs by name; this records the cause.
		log.Printf("[OOB DELETE] StartBuild evicting %d stale output(s) for missing blob %s", len(queue[digest]), digest.String())
...
func (d *BazelOutputServiceDirectory) stageSingleArtifact(ctx context.Context, artifact *bazeloutputservice.StageArtifactsRequest_Artifact, outputPathState *outputPathState, buildState *buildState) error {
	...
	// [OOB CONFIRM] Log the IDENTITY (inode) of the directory this out-of-band
	// create writes into. Compare with the bridge's "[OOB CONFIRM] lookup ...
	// parentInode=" for the same path: a mismatch proves the kernel's cached
	// parent handle points at a stale (prior) output-path tree.
	var parentAttrs virtual.Attributes
	outputParentCreator.stack.Peek().VirtualGetAttributes(ctx, virtual.AttributesMaskInodeNumber, &parentAttrs)
	log.Printf("[OOB CONFIRM] create name=%q parentInode=%d", name.String(), parentAttrs.GetInodeNumber())

	// HACK(test): after creating this output out of band, perturb the EXACT name
	// THROUGH THE MOUNT to evict the kernel's stale NEGATIVE dentry for it (Bazel
	// probes the output before staging → kernel caches ENOENT → stat keeps serving
	// it). We open(O_CREATE) the artifact itself (the `.sentinel` placeholder
	// couldn't evict siblings' negatives). The kernel, holding the negative, sends
	// a create to the extension; the bridge's CreateItem passes
	// existingOptions=nil, so an already-staged file returns EEXIST instead of
	// being overwritten — NON-DESTRUCTIVE — and the kernel drops the negative and
	// falls back to opening the existing file (no O_EXCL).
	//
	// SAFETY: this is only safe because (a) the bridge create is fail-if-exists,
	// and (b) we pass NO O_TRUNC / O_WRONLY / O_EXCL. Adding any of those, or
	// changing the bridge to overwrite-on-create, would DESTROY the staged output
	// (the trap the os.Remove deletion probe fell into). Path is hardcoded to the
	// mount layout (~/bb_clientd/outputs/<outputBaseID>/<artifact path>).
	defer func() {
		const mountOutputsRoot = "/Users/mauricio/bb_clientd/outputs"
		absArtifact := filepath.Join(mountOutputsRoot, outputPathState.outputBaseID.String(), artifact.Path)
		if f, err := os.OpenFile(absArtifact, os.O_CREATE, 0o644); err != nil {
			// EEXIST is the expected/healthy outcome (the bridge refused to
			// overwrite); anything else is interesting.
			log.Printf("[OOB TOUCH] open(O_CREATE) %s -> %v", absArtifact, err)
		} else {
			f.Close()
			log.Printf("[OOB TOUCH] open(O_CREATE) %s -> ok (no negative was cached)", absArtifact)
		}
		// Stat through the mount to see whether the name now resolves.
		if fi, err := os.Stat(absArtifact); err != nil {
			log.Printf("[OOB TOUCH] stat %s -> %v", absArtifact, err)
		} else {
			log.Printf("[OOB TOUCH] stat %s -> exists size=%d mode=%v", absArtifact, fi.Size(), fi.Mode())
		}
	}()
	...
	// OUT-OF-BAND: the Bazel Output Service creates this artifact directly on the
	// virtual tree (over gRPC), bypassing the FSKit mount — so the kernel never
	// sees the create and a cached negative lookup for this path stays stale.
	log.Printf("[OOB CREATE] %s (%T)", artifact.Path, anyLocator)
